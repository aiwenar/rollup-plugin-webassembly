...
var ast = require('@webassemblyjs/ast');
...
function analyze(code) {
    const ast$$1 = wasmParser.decode(code);
...
const importStmt = imports => Object.keys(imports).map(module =>
    `import * as ${rollupPluginutils.makeLegalIdentifier(module)} from '${module}';`
).join('\n');
...
const importObject = imports => '{ ' + Object.keys(imports).map(module =>
    `'${module}': ${rollupPluginutils.makeLegalIdentifier(module)}`
).join(', ') + ' }';
...
function WebAssembly(options={}) {
    ...
    if (sync === true) {
        ...
        syncFilter = rollupPluginutils.createFilter(sync, null);
    } else if (typeof sync === 'object') {
        syncFilter = rollupPluginutils.createFilter(sync.include, sync.exclude);
    } else {
